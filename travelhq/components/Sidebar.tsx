...
import {
  LayoutDashboard, PlusCircle, Map, Sparkles, CreditCard, Globe
} from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 bg-white border-r border-border flex-col shadow-sm min-h-screen">
      ...
      {/* Mobile Top Header */}
      <header className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2">
        ...
        <Link
          href="/new-trip"
          className="flex items-center gap-1.5 bg-blue-600 text-white text-sm font-semibold px-3 py-1.5 rounded-lg"
        >
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 flex items-center justify-around px-2 py-2 shadow-lg">
        {navItems.map(({ href, label, icon: Icon }) => {
