...
import { LayoutDashboard, PlusCircle, Map, Sparkles, CreditCard, Sun, Moon, X, Menu } from "lucide-react";
...
export default function Sidebar() {
  ...
  // ── HOME PAGE: No sidebar, just a floating nav bar ──
  if (isHomePage) {
    return (
      <>
        {/* Floating hamburger button on homepage (desktop) */}
        <button
          onClick={() => setDrawerOpen(true)}
          className="hidden md:flex fixed top-5 left-5 z-50 w-10 h-10 items-center justify-center rounded-xl transition-all hover:scale-105"
          style={{ background: "rgba(0,0,0,0.4)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)" }}
          aria-label="Open menu">
          <Menu className="w-5 h-5 text-white" />
        </button>

        {/* Mobile: hamburger in top-right on homepage */}
        <button
          onClick={() => setDrawerOpen(true)}
          className="md:hidden fixed top-4 right-4 z-50 w-9 h-9 flex items-center justify-center rounded-xl transition-all"
          style={{ background: "rgba(0,0,0,0.4)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)" }}
          aria-label="Open menu">
        ...
        <DrawerMenu open={drawerOpen} onClose={closeDrawer} pathname={pathname} />
      </>
    );
  }

  // ── INNER PAGES: Full sidebar on desktop, top+bottom nav on mobile ──
  return (
    <>
      {/* Desktop Sidebar */}
...
function DrawerMenu({ open, onClose, pathname }: { open: boolean; onClose: () => void; pathname: string }) {
  return (
    <>
      {/* Backdrop */}
      {open && (
      ...
      {/* Drawer panel */}
      <div
        className="fixed top-0 left-0 h-full z-50 flex flex-col w-72 transition-transform duration-300 ease-in-out"
        style={{
